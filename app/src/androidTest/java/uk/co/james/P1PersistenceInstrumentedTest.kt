...
class P1PersistenceInstrumentedTest {
    @Test fun releasedSchemaMigratesWithoutDataLoss(){
        ...
        val migrated=Room.databaseBuilder(context,JamesDatabase::class.java,name).addMigrations(MIGRATION_1_2,MIGRATION_2_3,MIGRATION_3_4,MIGRATION_4_5,MIGRATION_5_6,MIGRATION_6_7).build()
    ...
    @Test fun versionFiveNullableLegacyPayloadSurvivesMigrationAndIsQuarantined() {
        ...
        android.database.sqlite.SQLiteDatabase.openOrCreateDatabase(file,null).use {legacy->
            ...
            legacy.execSQL("CREATE INDEX index_records_timestamp ON records(timestamp)")
            legacy.execSQL("CREATE INDEX index_records_source_externalId ON records(source,externalId)")
            legacy.execSQL("CREATE INDEX index_records_kind_timestamp ON records(kind,timestamp)")
            legacy.execSQL("CREATE INDEX index_records_source_kind_timestamp ON records(source,kind,timestamp)")
            legacy.execSQL("CREATE INDEX index_records_store_timestamp ON records(store,timestamp)")
            legacy.execSQL("CREATE INDEX index_records_algorithmId_timestamp ON records(algorithmId,timestamp)")
            legacy.execSQL("CREATE INDEX index_records_algorithmId_calibrationVersion_timestamp ON records(algorithmId,calibrationVersion,timestamp)")
            legacy.execSQL("CREATE INDEX index_records_candidateStatus_timestamp ON records(candidateStatus,timestamp)")
            legacy.execSQL("CREATE INDEX index_records_jamesDayId_algorithmId_timestamp ON records(jamesDayId,algorithmId,timestamp)")
            legacy.execSQL("CREATE INDEX index_records_jamesDayId_timestamp ON records(jamesDayId,timestamp)")
            legacy.execSQL("CREATE TABLE archives (id TEXT NOT NULL PRIMARY KEY, path TEXT NOT NULL, digest TEXT NOT NULL, createdAt TEXT NOT NULL, purpose TEXT NOT NULL)")
            legacy.execSQL("CREATE TABLE import_history (id TEXT NOT NULL PRIMARY KEY, source TEXT NOT NULL, timestamp TEXT NOT NULL, found INTEGER NOT NULL, added INTEGER NOT NULL, duplicates INTEGER NOT NULL, conflicts INTEGER NOT NULL, errors TEXT NOT NULL, originalArchiveId TEXT NOT NULL)")
        ...
        val migrated=Room.databaseBuilder(context,JamesDatabase::class.java,name).addMigrations(MIGRATION_5_6,MIGRATION_6_7).build()
        try {runBlocking {
    ...
    @Test fun versionSixShiftFactsGainIndexedProvenanceColumnWithoutLoss() {
        val context=ApplicationProvider.getApplicationContext<Context>()
        val name="shift-provenance-${System.nanoTime()}.db"
        val file=context.getDatabasePath(name).apply {parentFile?.mkdirs()}
        android.database.sqlite.SQLiteDatabase.openOrCreateDatabase(file,null).use { legacy->
            legacy.execSQL("CREATE TABLE records (store TEXT NOT NULL, recordId TEXT NOT NULL, kind TEXT NOT NULL, source TEXT NOT NULL, timestamp TEXT NOT NULL, localDate TEXT NOT NULL, updatedAt TEXT NOT NULL, externalId TEXT, rawJson TEXT, algorithmId TEXT, calibrationVersion TEXT, candidateStatus TEXT, jamesDayId TEXT, PRIMARY KEY(store,recordId))")
            legacy.execSQL("CREATE INDEX index_records_kind ON records(kind)")
            ...
            legacy.execSQL("CREATE TABLE archives (id TEXT NOT NULL PRIMARY KEY, path TEXT NOT NULL, digest TEXT NOT NULL, createdAt TEXT NOT NULL, purpose TEXT NOT NULL)")
            legacy.execSQL("CREATE TABLE import_history (id TEXT NOT NULL PRIMARY KEY, source TEXT NOT NULL, timestamp TEXT NOT NULL, found INTEGER NOT NULL, added INTEGER NOT NULL, duplicates INTEGER NOT NULL, conflicts INTEGER NOT NULL, errors TEXT NOT NULL, originalArchiveId TEXT NOT NULL)")
            legacy.execSQL("INSERT INTO records (store,recordId,kind,source,timestamp,localDate,updatedAt,rawJson) VALUES ('personalRecords','shift-tracker-work:legacy','OwnershipPeriod','shift_tracker','2026-09-15T12:00:00Z','2026-09-15','2026-09-15T12:00:00Z','{\"data\":{\"externalShiftId\":\"legacy-shift\"}}')")
            legacy.version=6
        }
        val migrated=Room.databaseBuilder(context,JamesDatabase::class.java,name).addMigrations(MIGRATION_6_7).build()
        try {runBlocking {
            val row=migrated.records().get("personalRecords","shift-tracker-work:legacy")
            assertNotNull(row)
            // Older rows deliberately use the narrow provider-retraction
            // fallback; the migration must preserve the source facts exactly.
            assertNull(row!!.sourceShiftId)
            assertEquals("legacy-shift",row.data().text("externalShiftId"))
        }} finally {migrated.close();context.deleteDatabase(name)}
    }
}
