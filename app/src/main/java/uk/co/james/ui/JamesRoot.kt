...
@Composable fun JamesRoot(vm: JamesViewModel,chooseImport:()->Unit,export:(String?)->Unit,requestHealth:(Set<String>)->Unit,requestLocation:()->Unit,requestActivity:()->Unit,openAppSettings:()->Unit,install:()->Unit) {
    ...
    Scaffold(containerColor=MaterialTheme.colorScheme.background,contentColor=MaterialTheme.colorScheme.onBackground,topBar={TopAppBar(title={Text(if(route in listOf("Today","Timeline","Me","Insights","Nova"))"James OS"else route.substringAfter("Settings:",route),fontWeight=FontWeight.ExtraBold)},navigationIcon={if(route!=tab)IconButton(onClick={vm.navigate(backRoute)}){Icon(Icons.Outlined.ArrowBack,"Back")}},actions={IconButton(onClick={vm.navigate("Settings")}){Icon(Icons.Outlined.Settings,"Settings")}},colors=TopAppBarDefaults.topAppBarColors(containerColor=MaterialTheme.colorScheme.background,titleContentColor=MaterialTheme.colorScheme.onBackground,navigationIconContentColor=MaterialTheme.colorScheme.onSurfaceVariant,actionIconContentColor=MaterialTheme.colorScheme.onSurfaceVariant))},bottomBar={Column {HorizontalDivider(color=MaterialTheme.colorScheme.outlineVariant);NavigationBar(containerColor=MaterialTheme.colorScheme.surface,tonalElevation=0.dp) {listOf("Today" to Icons.Outlined.WbSunny,"Timeline" to Icons.Outlined.Schedule,"Me" to Icons.Outlined.PersonOutline,"Insights" to Icons.Outlined.Insights).forEach {(name,icon)->NavigationBarItem(selected=tab==name,onClick={vm.navigate(name,true)},icon={Icon(icon,null)},label={Text(name)},colors=navColors)}}}},snackbarHost={SnackbarHost(snack)}) {padding->
