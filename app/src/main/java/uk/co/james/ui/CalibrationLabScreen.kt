...
private fun CalibrationAlgorithm(vm:JamesViewModel,rows:List<uk.co.james.database.StoredRecord>,events:List<CalibrationObservation>,algorithmId:String) {
    ...
    AdaptiveCards(buildList {
        ...
        add {JamesCard("Observation history","MOST RECENT") {
            ...
            events.sortedByDescending {it.timestamp}.take(30).forEach {event->
                Text(event.timestamp.toString()+" · prediction "+event.prediction.roundToInt()+" · James "+event.observed.roundToInt())
    ...
    if(calibrating&&currentTarget!=null) AlertDialog(onDismissRequest={calibrating=false;calibrationTargetAtOpen=null},title={Text("How does this compare?")},text={Column(verticalArrangement=Arrangement.spacedBy(8.dp)) {
        Text("James OS prediction: "+currentTarget.score)
