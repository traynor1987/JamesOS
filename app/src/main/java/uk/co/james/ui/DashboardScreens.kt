...
@Composable private fun CurrentContextCard(vm:JamesViewModel,summary:ContextLoadSummary) {
    ...
    JamesCard("Current context","EXPERIMENTAL · PRIVATE") {
        ...
        if(active==null) {
            ...
            Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(8.dp)) {
                listOf("PERSONAL","NEUTRAL","OBLIGATION").forEach {type->OutlinedButton(onClick={vm.startContext(type)},modifier=Modifier.weight(1f)){Text(type,style=MaterialTheme.typography.labelSmall)}}
            }
        } else {
            Text(active.placeName?:"CURRENT PERIOD",fontWeight=FontWeight.Bold)
            Text(active.visitType.name,fontWeight=FontWeight.Black,color=jamesLime)
            ...
            if(chooser)Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(6.dp)) {listOf("PERSONAL","NEUTRAL","OBLIGATION").forEach {type->TextButton(onClick={vm.endContext();vm.startContext(type)}){Text(type)}}}
