...
@Composable fun RecordEditor(vm:JamesViewModel) {
    ...
    AlertDialog(onDismissRequest={if(!busy)vm.close()},title={Text(if(kind=="RutEvent")"Edit life event"else kind)},text={Column(Modifier.verticalScroll(rememberScrollState()),verticalArrangement=Arrangement.spacedBy(10.dp)) {
        when(kind) {
            ...
            "PlaceVisit" -> {field("title","Place");Choice("Category",d.text("category","Unclassified"),listOf("Home","Work","Family","Shopping","Gym","Food","Leisure","Health / appointment","Personal","Other","Unclassified")){vm.change("category",p(it))};Choice("Time ownership",d.text("ownership","UNKNOWN"),listOf("UNKNOWN","AUTONOMOUS","COMMITTED","CONSTRAINED","WORK")){vm.change("ownership",p(it));vm.change("ownershipSource",p("JAMES_CORRECTION"))};field("activity","What did you do?");field("note","Notes");TextButton(onClick={vm.addVisitInterruption(raw.text("id"),"OTHER")}){Text("Record interruption")};TextButton(onClick={vm.action {vm.repo.dao.get("personalRecords",raw.text("id"))?.let(vm::splitVisit)}}){Text("Split visit")};TextButton(onClick={vm.action {vm.repo.dao.get("personalRecords",raw.text("id"))?.let(vm::mergeVisitWithPrevious)}}){Text("Merge with previous")};Muted("Arrival, departure and place are estimated from battery-aware evidence. Time ownership is your correction; Home and no-work are never assumed to be personal time.")}
