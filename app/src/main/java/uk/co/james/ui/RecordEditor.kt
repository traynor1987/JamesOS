...
@Composable fun RecordEditor(vm:JamesViewModel) {
    ...
    AlertDialog(onDismissRequest={if(!busy)vm.close()},title={Text(if(kind=="RutEvent")"Edit life event"else kind)},text={Column(Modifier.verticalScroll(rememberScrollState()),verticalArrangement=Arrangement.spacedBy(10.dp)) {
        when(kind) {
            ...
            "ContextPeriod" -> {Choice("Situation",d.text("visitType","UNKNOWN"),listOf("UNKNOWN","HOME","WORK","DRIVING","SHOPPING","FAMILY","SOCIAL","APPOINTMENT","ERRAND","MEAL","LEISURE","PERSONAL_PROJECT","RESTING","OTHER")){vm.change("visitType",p(it))};field("start","Started (ISO 8601)");field("end","Ended (leave blank if current)");field("note","Notes");Muted("Context answers what situation is happening. It does not classify whose time it is.")}
            "LifeFactActivity" -> {field("title","What did you do?");Choice("Activity type",d.text("activityType","OTHER"),listOf("GAMING","CODING","WATCHING","EATING","WALKING","GYM","SHOPPING","DRIVING","HELPING","HOUSEWORK","APPOINTMENT","DELIVERY_WORK","RESTING","OTHER")){vm.change("activityType",p(it))};field("start","Started (ISO 8601)");field("end","Ended (leave blank if current)");field("note","Notes");Muted("Activity answers what you did. It does not automatically create Personal time.")}
            "PlaceVisit" -> {Text("${d.text("start").ifBlank {"Unknown arrival"}} → ${d.text("end").ifBlank {"Current visit"}}",style=MaterialTheme.typography.bodySmall);Text("Duration: ${d.number("durationMin").toLong()}m · ${d.text("provenance",d.text("ownershipSource","INFERRED")).lowercase().replace('_',' ')}",style=MaterialTheme.typography.bodySmall);field("title","Place");Choice("Category",d.text("category","Unclassified"),listOf("Home","Work","Family","Shopping","Gym","Food","Leisure","Health / appointment","Personal","Other","Unclassified")){vm.change("category",p(it))};Choice("Context",d.text("context","UNKNOWN"),listOf("UNKNOWN","HOME","WORK","DRIVING","SHOPPING","FAMILY","SOCIAL","APPOINTMENT","ERRAND","LEISURE","PERSONAL_PROJECT","RESTING","OTHER")){vm.change("context",p(it));vm.change("contextSource",p("JAMES_CORRECTION"))};Choice("Time ownership",d.text("ownership","UNKNOWN"),listOf("UNKNOWN","AUTONOMOUS","COMMITTED","CONSTRAINED","WORK")){vm.change("ownership",p(it));vm.change("ownershipSource",p("JAMES_CORRECTION"))};field("activity","What did you do?");field("note","Notes");TextButton(onClick={vm.interruptCurrentTime("OTHER")}){Text("INTERRUPTED NOW")};TextButton(onClick={vm.action {vm.repo.dao.get("personalRecords",raw.text("id"))?.let(vm::splitVisit)}}){Text("Split visit")};TextButton(onClick={vm.action {vm.repo.dao.get("personalRecords",raw.text("id"))?.let(vm::mergeVisitWithPrevious)}}){Text("Merge with previous")};Muted("Place, context, activity and time ownership are separate. Arrival/departure and the original GPS evidence remain intact when you correct this visit.")}
