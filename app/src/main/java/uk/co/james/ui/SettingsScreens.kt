...
@Composable private fun MentalWellbeingSettingsCard(vm:JamesViewModel) {
    ...
    JamesCard("Mental wellbeing","Private experimental insights · not a diagnosis") {
        ...
        SensorSwitch("Use Rut",current.rut){value->save {settings->settings.copy(rut=value)}}
...
@Composable fun ConnectionsScreen(vm:JamesViewModel,requestHealth:(Set<String>)->Unit) {
...
@Composable fun LocationScreen(vm:JamesViewModel,records:List<StoredRecord>,requestLocation:()->Unit,requestActivity:()->Unit,settings:()->Unit) {
    ...
    AdaptiveCards(listOf(
        ...
        {JamesCard("Tracking & diagnostics",if(allDay)"Low-power tracking active"else "Off") {
            ...
            current?.let {anchor->val d=anchor.data();val activeOwnership=records.filter {it.kind=="OwnershipPeriod"&&it.data().text("end").isBlank()}.maxByOrNull {it.timestamp};HorizontalDivider(Modifier.padding(vertical=8.dp));Text("Current place: ${d.text("placeName","Unknown place")}");Muted("Here ${visitTime(d.number("durationMin").toLong())} · ${d.text("movement","Stationary")} · accuracy ${d.number("accuracy").toInt()}m");activeContext?.data()?.text("visitType")?.let {Muted("Context: ${it.lowercase().replace('_',' ').replaceFirstChar(Char::uppercase)}")};Muted("Time ownership: ${activeOwnership?.data()?.text("ownership")?:"UNKNOWN"} · ${activeOwnership?.data()?.text("ownershipSource")?:"No confirmation"}");listOf("AUTONOMOUS" to "MY TIME","COMMITTED" to "OBLIGATION","CONSTRAINED" to "CONSTRAINED","WORK" to "WORK").chunked(2).forEach {row->Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(8.dp)){row.forEach {(value,label)->TextButton(onClick={vm.setCurrentOwnership(value)},modifier=Modifier.weight(1f)){Text(label,style=MaterialTheme.typography.labelSmall)}}}}}
        }},
