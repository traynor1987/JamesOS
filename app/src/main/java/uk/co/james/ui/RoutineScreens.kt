...
@Composable fun RoutinesScreen(vm:JamesViewModel,records:List<StoredRecord>) {
    ...
    LazyColumn(contentPadding=PaddingValues(16.dp),verticalArrangement=Arrangement.spacedBy(16.dp)) {
        ...
        item {JamesCard("Your RUT journey"){Button(onClick={vm.navigate("Life events")}){Text("Life events & recovery")}}}
    }
}
@Composable fun RutScreen(vm:JamesViewModel,records:List<StoredRecord>,route:String) {
    val date by vm.date.collectAsStateWithLifecycle();var category by rememberSaveable {mutableStateOf("All")};var query by rememberSaveable {mutableStateOf("")};var initial by rememberSaveable {mutableStateOf("0")}
    ...
    LazyColumn(contentPadding=PaddingValues(16.dp),verticalArrangement=Arrangement.spacedBy(16.dp)) {
        item {PageTitle(route,"YOUR RUT HISTORY, CONTINUED");Choice("Explore",route,listOf("Life events","Journey","Event history","Recovery space","Daily notes","Calendar","RUT Insights","Manage events")){vm.navigate(it)};DateControl(date,vm::date)}
        if(events.isEmpty())item {JamesCard("Continue your journey") {Button(onClick={vm.navigate("Import Centre")}){Text("Import existing RUT data")};Muted("Only start a new journey if you have no existing history.");OutlinedTextField(initial,{initial=it},label={Text("Starting score")});Button(enabled=initial.toLongOrNull()!=null,onClick={vm.start(initial.toLong())}){Text("Start a new journey")}}}
        item {JamesCard(Ledger.stage(Ledger.score(raw)),"${Ledger.score(raw)} points"){Text("${events.size} historical entries · ${recovered.size} recoveries")}}
        when(route) {
            "Life events","Manage events" -> {
                item {Choice("Category",category,listOf("All")+templates.map {it.raw().text("category")}.distinct().sorted()){category=it};OutlinedTextField(query,{query=it},label={Text("Find event")},modifier=Modifier.fillMaxWidth());if(route=="Manage events")Button(onClick={vm.open("Template")}){Text("Add event definition")}}
                items(templates.filter {(route=="Manage events"||it.raw().flag("enabled"))&&(category=="All"||it.raw().text("category")==category)&&it.raw().text("title").contains(query,true)}.sortedBy {it.raw().number("order")},key={it.recordId}) {t->JamesCard("${t.raw().text("emoji")} ${t.raw().text("title")}","${t.raw().number("points").toInt()} · ${t.raw().text("category")}") {if(route=="Manage events"){TextButton(onClick={vm.open("Template",t)}){Text("Edit definition")};TextButton(onClick={vm.action {vm.repo.save(t.store,t.raw().changed("enabled" to p(!t.raw().flag("enabled"))),t.rawJson)}}){Text(if(t.raw().flag("enabled"))"Disable"else "Enable")}}else Button(enabled=events.isNotEmpty(),onClick={vm.log(t)}){Text("Log event")};TextButton(onClick={vm.pin(t)}){Text("Toggle favourite")}}}
            }
            "Recovery space" -> items(events.filter {it.raw().text("type")=="negative"&&it.recordId !in recovered},key={it.recordId}) {e->JamesCard(e.raw().text("title"),e.localDate){Text(e.raw().text("recoveryTitle","Take one small recovery step"));if(e.recordId !in closed)Button(onClick={val t=templates.find {it.recordId==e.raw().text("templateId")};vm.log(t?:e,e)}){Text("Record recovery")};TextButton(onClick={vm.setAside(e,e.recordId !in closed)}){Text(if(e.recordId in closed)"Reopen"else "Set aside")}}}
