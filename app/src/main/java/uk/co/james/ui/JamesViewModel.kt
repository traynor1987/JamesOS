...
class JamesViewModel(application: Application,private val saved: SavedStateHandle): AndroidViewModel(application) {
    ...
    fun confirmCurrentPlace(place:StoredRecord)=action {
        app.location.confirmCurrentPlace(place.recordId)
        message.value="Current visit confirmed as ${place.data().text("title")}. Time ownership is unchanged."
    }
