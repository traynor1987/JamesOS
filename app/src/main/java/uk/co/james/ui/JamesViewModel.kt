...
class JamesViewModel(application: Application,private val saved: SavedStateHandle): AndroidViewModel(application) {
    ...
    val health=MutableStateFlow(HealthStatus(false,emptySet(),"Checking availabilityâ€¦"))
    ...
    fun saveCurrentPlace(name:String,category:String)=action {
        placeCalibration.value=PlaceCalibrationUiState("Checking current locationâ€¦","Using an existing fix immediately only when it is fresh and accurate.",true)
        try {
            val fix=app.location.addCurrentPlace(name,category) {
                placeCalibration.value=PlaceCalibrationUiState("Getting precise locationâ€¦","Passive tracking is low power; calibration is requesting one fresh high-accuracy fix.",true)
            }
            val detail=fix.diagnostic(java.time.Instant.now())
            placeCalibration.value=PlaceCalibrationUiState("Place saved",detail,false)
            message.value="Saved ${name.trim()} Â· $detail"
        } catch(e:Exception) {
    ...
    fun open(kind: String,entry: StoredRecord?=null) {
        ...
        val raw=entry?.raw()?:when(kind) {
            ...
            "Template" -> fields("id" to p(id()),"title" to p(""),"emoji" to p("âœ¨"),"points" to p(25),"category" to p("Other"),"type" to p("positive"),"enabled" to p(true),"isDefault" to p(false),"order" to p(records.value.count {it.store=="eventTemplates"}))
    ...
    fun calibrate(algorithmId:String,target:CalibrationTarget,submissionId:String,feedback:String,note:String="",capacity:String?=null,sleepiness:String?=null)=action {
        ...
        message.value="Calibration saved. Observation #$count â€” James OS will compare it with similar observations."
    }
    ...
    fun updateCheck()=action {
        ...
        updateProgress.value="Checkingâ€¦"
