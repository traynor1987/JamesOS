...
/** Device-only GitHub credential. Never included in James exports or Android backup. */
class UpdateCredentials(context:Context) {
    private val file=AtomicFile(File(context.noBackupFilesDir,"github-update-access"))
    private val alias="james-github-update-access-v1"
    ...
    @Synchronized fun read():String {
        ...
        }catch(e:Exception){throw IllegalStateException("GitHub access could not be unlocked. Remove it and save a new token.")}
    }
    @Synchronized fun save(value:String) {
        val token=value.trim();require(token.length in 20..1024 && token.none {it.isWhitespace()}) {"Enter a valid GitHub access token."}
