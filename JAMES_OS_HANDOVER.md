...
- Release: https://github.com/traynor1987/JamesOS/releases/tag/v0.3.231

## Current-place authority and place learning (2026-09-17)

Geofence events are now first-class **place** evidence. An `Arrived at Home` event creates/updates the single active `LocationAnchor` as **Home · CONFIRMED**, closes the prior active place when needed, and ignores a late exit callback for that old place. It does not set Time Ownership: Home can remain Unknown, Personal, Committed, Constrained or Work according to separate authoritative ownership facts.

The Location screen distinguishes geofence place evidence from a passive location point. A stale cached passive point is rejected as current evidence; when retained for diagnostics it is labelled as the last passive fix rather than current accuracy. The active Visit/anchor survives restart and is the current-place authority until a valid departure occurs.

When passive evidence is Unknown or ambiguous, Places offers a compact **Where are you?** choice of existing saved places. Choosing Home repoints the current anchor and stores a `PlaceIdentityCorrection`; it never creates a duplicate Place or changes ownership. Accurate repeated manual confirmations can learn a secondary matching centre after three samples, without expanding the saved geofence radius or merging distinct nearby places.
...
[JAMES_OS_WHOOP_FIELD_AUDIT.md](JAMES_OS_WHOOP_FIELD_AUDIT.md) is the source-traced Part 1 audit of current WHOOP scopes, raw retention, mapped fields, provenance, health-source overlap and supported public API fields. Current utilisation is strong for Recovery, sleep/Performance, overnight HRV/RHR and cycle Strain. The audit’s original Part 2 proposal is implemented in the status section below; workouts/HR zones remain unmapped Part 3 work.

### WHOOP Sleep Detail — Part 2 (2026-09-15)

Part 2 maps only scored WHOOP Sleep raw evidence into one `SleepDetail` record per provider sleep (`whoop:sleep-detail:<id>`, mapping version `1`): baseline need, sleep-debt/recent-strain/signed-nap components, transparent total need, shortfall/met/surplus, efficiency, consistency, awake time, disturbances, time-in-bed/no-data/cycle count and Light/SWS/REM composition. Raw `ExternalRecord.data.original` stays immutable; provenance includes WHOOP record/cycle/times/update/ingest/mapping version. A bounded one-time local backfill maps retained raw records; regular provider revisions update the same identity. No extra WHOOP calls/scopes/cadence and no duplicate main Sleep records.
...
### Life Balance v2 / Legacy RUT retirement — 2026-09-16

[JAMES_OS_BALANCE_SUCCESSION_AUDIT.md](JAMES_OS_BALANCE_SUCCESSION_AUDIT.md) now records the shipped `life_balance_v2` v2.0.0 boundary. Current Life Balance is a deterministic bounded 0–100 ownership/alignment interpretation over 7/28/90-day windows (28-day primary once sufficiently covered); it consumes only Time Ownership, ownership coverage and interruption continuity. Unknown lowers coverage rather than score; Work/Committed are factual, not automatic penalties; health, Low Mood, Time Pressure, GPS/place, activity labels, routines and Legacy RUT are not inputs. The calculation is rebuilt from bounded factual records, so backup/import remains the existing generic record path and no derived-output loop/migration is introduced.

Legacy RUT is retired from normal current terminology but fully preserved as **Legacy RUT history**: ledger events, templates, recovery links, notes, milestones, settings and import provenance remain unchanged and never numerically feed Balance v2. Life Balance v1.0.1 remains available as the version-compatible contextual calculation for Low Mood v1.3.1 and Mental Reserve, so v2 installation does not change them. Direct weekly `life_balance_v2` calibration uses an unambiguous “how much has your time/life felt like your own?” scale once automatic coverage supports a prediction; feedback remains separate from ownership facts.
...
### Canonical Focus mark and current ownership lifecycle (2026-09-15)

- `branding/focus-mark.svg` is the canonical Focus geometry traced from the approved artwork. It is a broken outer ring, one straight vertical stem and one circular terminal—**not** a typographic J. Phone header/splash/adaptive foreground/themed/notification resources reference `ic_james_focus_symbol`; Wear uses the identical 108-unit geometry in its one Wear master. `FocusMarkAssetTest` guards the no-hook path contract and prevents independent variants drifting.
- Current Time Ownership is now a real lifecycle: a visible active **PERSONAL / OBLIGATION / CONSTRAINED / WORK** segment shows its live duration, CHANGE and an explicit END action. Ending closes the completed period and atomically opens `UNKNOWN` from that instant, so prior Personal time stops increasing. A direct change closes/open successors in one Room transaction; interruption/resume remain separate linked semantics and commit their related records atomically.
- The local ownership duration clock only redraws the small control every 30 seconds. It does not recalculate history; Today, Life Balance, Timeline and Visit detail continue to consume the canonical ownership ledger.
