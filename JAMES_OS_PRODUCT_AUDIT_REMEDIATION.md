...
| Today preparation NPE | The component boundary contained a transient NPE but normal delayed provider state could still throw | A historical sleep baseline with no current main sleep is an explicit low-confidence learning state; contained failure reports now retain the preparation stage, message and first James frame locally | Resolved in code; exact historical-only regression test and real-device confirmation required |
| StoredRecord legacy payload parsing | Lazy parse-once preparation assumed every historical `rawJson` value was a non-null JSON object; Fold diagnostic localized a remaining NPE to that assumption | Database v6 preserves nullable historical payloads; parse-once classification quarantines missing, empty, malformed and non-object rows from semantics while retaining identity/provenance and backup sidecar round-trip | Resolved in code; v5→v6 migration, parser, backup and Insights→Today mature-fixture regression coverage |
| Today reactive refresh | Every `produceState` run set the whole route back to Loading; persisted derived outputs could invalidate their own source observer | Explicit initial/ready/refreshing/error state retains the last coherent snapshot, while derived snapshots are excluded from their own bounded input stream | Resolved in code; state-machine and derived-input regression tests; long real-device soak required |
...
Contained Today preparation failures also use this report with a `today_preparation` stage. This led to the concrete sleepiness root cause: historical Sleep evidence may be sufficient for a baseline before a current main Sleep record arrives. That is `LOW` confidence/learning, not a null current record. The fix preserves historical inputs and does not create or persist a default observation.
